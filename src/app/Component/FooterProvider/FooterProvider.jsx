...
const FooterProvider = ({ initialDoctors, initialFooter }) => {
  ...
  // Polling function to fetch updated data every 5 seconds
  useEffect(() => {
    const fetchData = async () => {
      try {
        const updatedDoctors = await fetchDoctors();
        const updatedFooter = await fetchFooterData();
        if (JSON.stringify(updatedDoctors) !== JSON.stringify(doctors)) {
          setDoctors(updatedDoctors);
        }

        if (JSON.stringify(updatedFooter) !== JSON.stringify(footer)) {
          setFooter(updatedFooter);
        }
      } catch (error) {
        console.error("Error fetching footer or doctor data:", error);
      }
    };
