...
const Hero = ({ heroSection }) => {
   const { i18n } = useTranslation();
   const [section, setSection] = useState(heroSection);
  ...
  useEffect(() => {
    const fetchUpdatedHero = async () => {
      try {
        ...
        setSection((prev) => (data && data.heroSection ? data.heroSection : prev));
      } catch (error) {
        console.error("Error fetching updated hero:", error);
      }
    };

    const interval = setInterval(fetchUpdatedHero, 5000); // poll every 5s
    return () => clearInterval(interval); // cleanup
  }, []);

  // Resolve current translation
  const currentLanguage = i18n.language || "en";
  const tr = useMemo(
    () => (section && section.translations && section.translations[currentLanguage]) || {},
  ...
  const heroImage =
    bgPath ? `${BASE}/${bgPath}${bgPath.includes("?") ? "&" : "?"}t=${Date.now()}` : "";
