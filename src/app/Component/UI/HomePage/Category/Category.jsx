...
const Category = ({ departments, locale }) => {
  ...
  // State to hold the departments data
  const [categoryData, setCategoryData] = useState(departments || []);
  const [pollingKey, setPollingKey] = useState(Date.now());

  useEffect(() => {
    const interval = setInterval(async () => {
      try {
        const updatedDepartments = await fetchDepartments(currentLanguage);
        if (JSON.stringify(updatedDepartments) !== JSON.stringify(categoryData)) {
          setCategoryData(updatedDepartments);
          setPollingKey(Date.now());
        }
      } catch (error) {
        console.error("Error fetching updated departments:", error);
      }
    }, 5000);

    return () => clearInterval(interval);
  }, [categoryData, currentLanguage]);

  // Show only the first 12 departments
  const displayedDepartments = categoryData?.slice(0, 12) || [];
  ...
  return (
    <div className="bg-[#E6F5F3] py-12 lg:py-[100px] px-[10px]" key={pollingKey}> {/* Force re-render */}
      <div className="container">
        {/* Section Heading */}
        <SectionHeading align="center" heading={t('category.title')} subtitle={t('category.subtitle')} />
