...
const MobileApp = ({ downloadAppSection }) => {
   const { t, i18n } = useTranslation();
  const currentLanguage = i18n.language || "en";
  const [section, setSection] = useState(downloadAppSection || null);

  useEffect(() => {
    const fetchUpdatedSection = async () => {
      try {
        const data = await fetchDynamicData(currentLanguage);
        if (data && data.downloadAppSection) {
          setSection(data.downloadAppSection);
        }
      } catch (error) {
        console.error("Error fetching updated MobileApp data:", error);
      }
    };
