...
const WhyChooseUs = ({ whyChooseUsSection }) => {
  ...
  // State to hold section data
  const [section, setSection] = useState(whyChooseUsSection || null);

  useEffect(() => {
    const fetchUpdatedSection = async () => {
      try {
        const data = await fetchDynamicData
        (currentLanguage);
        if (data && data.whyChooseUsSection) {
          setSection(data.whyChooseUsSection);
        }
      } catch (error) {
        console.error("Error fetching updated WhyChooseUs data:", error);
      }
    };
    ...
    // Poll every 5 seconds after the first fetch
    const interval = setInterval(fetchUpdatedSection, 5000);

    // Clean up on component unmount
    return () => clearInterval(interval);
  }, [currentLanguage]);

  const translations = section?.translations?.[currentLanguage] || {};
