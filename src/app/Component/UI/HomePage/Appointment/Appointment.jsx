// Appointment.jsx (FIXED)

"use client";

import React, { useState, useEffect } from "react"; // Form-এর জন্য useState/useEffect থাকবে
...
const Appointment = ({ appointmentSection }) => { // ✅ prop ব্যবহার করুন
  const { t, i18n } = useTranslation();
  const currentLanguage = i18n.language || "en";
  const { user } = useAuth() || {};
  // 🔴 useState(appointmentSection) বাদ

  // 🔴 setInterval লুপ সম্পূর্ণ মুছে ফেলা হয়েছে
  
  // ✅ সরাসরি 'appointmentSection' prop ব্যবহার করুন
  const tr = appointmentSection?.translations?.[currentLanguage] || {};

  const BASE = (process.env.NEXT_PUBLIC_BACKEND_URL || "").replace(/\/+$/, "");
  const rawPath = (tr.image || "").replace(/\\/g, "/").replace(/^\/+/, "");
  const appointmentImage = rawPath ? `${BASE}/${rawPath}` : "/assets/appointment-fallback.png";

  // --- Form Logic (এটি ঠিক আছে) ---
  ...
  }, [user]); // এই useEffect শুধু user ফর্মের জন্য, এটি ঠিক আছে।
