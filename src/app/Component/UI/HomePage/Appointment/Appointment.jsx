...
const Appointment = ({ appointmentSection }) => {
  ...
  const currentLanguage = i18n.language || "en";
  const { user } = useAuth() || {};
  const [section, setSection] = useState(appointmentSection || null);

  useEffect(() => {
    const fetchUpdatedAppointment = async () => {
      try {
        const data = await fetchDynamicData(currentLanguage);
        if (data && data.appointmentSection) {
          setSection(data.appointmentSection);
        }
      } catch (error) {
    ...
    // fetch once immediately, then every 5s
    fetchUpdatedAppointment();
    const interval = setInterval(fetchUpdatedAppointment, 5000);
    return () => clearInterval(interval);
  }, [currentLanguage]);
  
  const tr = section?.translations?.[currentLanguage] || {};
