...
const HeaderProvider = ({ header, departments }) => {
  ...
  // Add a useEffect to update isMounted.
  ...
  if (!isMounted || !header || !departments) {
    return (
...
/* Comment code is workable ssr and instant if I update in dashboard data update in frontend */
...
//       try {
//         const updatedHeader = await fetchHeaderData({ cache: "no-store" });
//         const updatedDepartments = await fetchDepartments();
//         setHeaderState(updatedHeader); // Update header data
//         setDepartmentsState(updatedDepartments); // Update departments data
//       } catch (error) {
//         console.error("Error fetching updated data:", error);
//       }
//     };

//     // Fetch the data immediately after mount and then every 10 seconds
//     fetchUpdatedData();
...
//   // Process menu items
//   const headerMenus =
//     headerState?.translations?.[currentLanguage]?.menus?.filter(
//       (menu) => menu.status === "active"
//     ) || [];
//   const sortedMenus = [...headerMenus].sort((a, b) => a.order - b.order);
//   const topMenus = sortedMenus.filter((menu) => !menu.parent);
...
//     href: "#",
//     hasSubMenu: true,
//     subMenus: departmentsState.map((dept) => ({
//       label:
//         dept.translations?.[currentLanguage]?.name ||
//         dept.translations?.en?.name ||
//         "Unnamed",
